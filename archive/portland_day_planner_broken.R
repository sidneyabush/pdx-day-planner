# portland_day_planner.R — Simple Day-Off Planner 🌟
# 
# SIMPLE WORKFLOW:
...
extract_coords_from_url <- function(url) {
  ...
  # Try different URL coordinate patterns in order of preference
  # Pattern 1: !3d!4d format (most common in saved places)
  ...
  # Pattern 2: !2d!3d format (alternative)
  matches <- str_match(url, "!2d(-?\\d+\\.?\\d*)!3d(-?\\d+\\.?\\d*)")
  if(!is.na(matches[1])) {
    lat <- as.numeric(matches[3])  # Note: lat and lng swapped in this format
    lng <- as.numeric(matches[2])
    if(!is.na(lat) && !is.na(lng)) {
      return(list(lat = lat, lng = lng))
    }
  }
  
  # Pattern 3: @lat,lng format (viewport)
  ...
  # Pattern 4: Query parameters ?q=lat,lng
  matches <- str_match(url, "[?&]q=(-?\\d+\\.?\\d*),(-?\\d+\\.?\\d*)")
  if(!is.na(matches[1])) {
    lat <- as.numeric(matches[2])
    lng <- as.numeric(matches[3])
    if(!is.na(lat) && !is.na(lng)) {
...
load_csv_data <- function() {
  ...
  # Normalize text to remove accents, umlauts, and special characters
  ...
  # Keep tags as-is since they contain useful emoji
  ...
  # Debug: Show coordinate extraction results
  valid_coords <- !is.na(all_data$lat) & !is.na(all_data$lng)
  cat("   Found coordinates for", sum(valid_coords), "out of", nrow(all_data), "places\n")
  
  # Show some examples of URLs that didn't work
  if(sum(!valid_coords) > 0) {
    failed_urls <- all_data$url[!valid_coords]
    cat("   ⚠️  Could not extract coordinates from", sum(!valid_coords), "places\n")
    if(length(failed_urls) > 0) {
      cat("   Example failed URL:", substr(failed_urls[1], 1, 100), "...\n")
    }
  }
  ...
  if(nrow(all_data) == 0) {
    cat("❌  No places found with valid coordinates!\n")
    cat("   This might happen if your CSV URLs are in a different format.\n")
    cat("   Please check that your Google Maps exports contain full URLs.\n")
    stop("No places found with valid coordinates")
  }
  ...
  # Validate coordinates are in reasonable range for Portland area
  portland_area <- abs(all_data$lat - 45.5) < 1 & abs(all_data$lng - (-122.7)) < 1
  if(sum(portland_area) < nrow(all_data) * 0.5) {
    cat("   ⚠️  Warning: Some coordinates seem outside Portland area\n")
  }
  
  cat("✅ Successfully processed", nrow(all_data), "places with valid coordinates\n")
...
ui <- fluidPage(
  ...
  div(class = "header",
    ...
    p(paste("Home base:", HOME_ADDRESS))
  ),
...
if(nrow(places) > 0) {
  ...
  cat("Please check your CSV files in the Saved/ folder.\n")
}