# Process ONLY Portland-specific CSV files
library(dplyr)
library(readr)
...
extract_coords_from_url <- function(url) {
  ...
  matches <- stringr::str_match(url, "!3d(-?\\\\d+\\\\.?\\\\d*)!4d(-?\\\\d+\\\\.?\\\\d*)")
  if(!is.na(matches[1])) {
  ...
  matches <- stringr::str_match(url, "@(-?\\\\d+\\\\.?\\\\d*),(-?\\\\d+\\\\.?\\\\d*)")
  if(!is.na(matches[1])) {
    lat <- as.numeric(matches[2])
    lng <- as.numeric(matches[3])
    if(!is.na(lat) && !is.na(lng)) return(list(lat = lat, lng = lng))
...
cat('\nPortland-only processing complete!\n')
cat('Total Portland places:', nrow(all_data), '\n')
cat('Lists processed: Hikes, Movies, Museums, Parks, Portland stuff, Food & Drink\n')
